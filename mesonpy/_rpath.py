...
class RPATH:
    ...
    @classmethod
    def fix_rpath(cls, filepath: Path, build_rpath: List[str], install_rpath: List[str]) -> None:
        old_rpath = cls.get_rpath(filepath)
        new_rpath = [path for path in old_rpath if path not in build_rpath]
        new_rpath.extend(install_rpath)
...
class _MacOS(RPATH):
    ...
    def fix_rpath(cls, filepath: Path, build_rpath: List[str], install_rpath: List[str]) -> None:
        ...
        def translate(path: str) -> str:
            ...
            if root == '$ORIGIN':
                path = f'@loader_path{sep}{stem}'
                # warnings.warn('...')
            return path

        super().fix_rpath(filepath, build_rpath, [translate(path) for path in install_rpath])
